...
A bank of modules (InnerDetectors)
"""
...
class ModuleArrayBuilder(gegede.builder.Builder):
    ...
    def configure(self, N_ModuleX, N_ModuleZ, TopLArHeight, TopHeight, **kwargs):
        self.Material   = 'LAr'
        ...
        # Subbuilders
        self.InnerDetector_builder = self.get_builder('NDBucket')
    ...
    def construct(self,geom):
        ...
        self.halfDimension  = { 'dx':   self.InnerDetector_builder.halfDimension['dx']*self.N_ModuleX,
                                'dy':   self.InnerDetector_builder.halfDimension['dy'] + self.TopLArHeight + self.TopHeight,
                                'dz':   self.Tub_builder.halfDimension['dy']}

        arraydz = self.InnerDetector_builder.halfDimension['dz']*self.N_ModuleZ
        ...
        Service_shape = geom.shapes.Box('Service_shape',
                                        dx=self.InnerDetector_builder.halfDimension['dx'],# - Q('5mm')/2,
                                        dy=self.TopHeight,
                                        dz=self.Tub_builder.halfDimension['dy'])
                                        #dz=(self.N_ModuleZ*self.InnerDetector_builder.halfDimension['dz']) - Q('5mm')/2)
        ...
        for i in range(self.N_ModuleX):
            ...
            for j in range(self.N_ModuleZ):
                pos = [
                    -self.halfDimension['dx']+(2*i+1)*self.InnerDetector_builder.halfDimension['dx'],
                    -self.halfDimension['dy']+self.InnerDetector_builder.halfDimension['dy'],
                    -arraydz+(2*j+1)*self.InnerDetector_builder.halfDimension['dz']
                ]

                Module_lv = self.InnerDetector_builder.get_volume()

                Module_pos = geom.structure.Position(self.InnerDetector_builder.name+'_pos_'+str(i)+'.'+str(j),
                                                        pos[0],pos[1],pos[2])

                Module_pla = geom.structure.Placement(self.InnerDetector_builder.name+'_pla_'+str(i)+'.'+str(j),
                                                        volume=Module_lv,
                                                        pos=Module_pos,
                                                        copynumber=2*j+i)
            ...
            pos = [
                -self.halfDimension['dx']+(2*i+1)*self.InnerDetector_builder.halfDimension['dx'],
                -self.halfDimension['dy']+2*self.InnerDetector_builder.halfDimension['dy']+self.Grating_builder.halfDimension['dx'],
                Q('0mm')
            ]
            ...
            pos = [
                -self.halfDimension['dx']+(2*i+1)*self.InnerDetector_builder.halfDimension['dx'],
                self.halfDimension['dy']-self.TopHeight,#+2*self.InnerDetector_builder.halfDimension['dy']+self.TopHeight,
